...
```
The ***mean*** number of steps taken per day is `r mean(activity$steps, na.rm = T)` steps.

```{r}
median(activity$steps, na.rm = T)
```
The ***median*** number of steps taken per day is `r median(activity$steps, na.rm = T)` steps.
...
```{r, cache=TRUE, cache.vars=TRUE, results='hide'}
library(mice)
...
```
The **mean** number of steps taken per day is `r mean(imp_activity$steps)`.

```{r}
median(imp_activity$steps)
```
The **median** number of steps taken per day is `r median(imp_activity$steps)`.
